...
## Project goals ledger (READ FIRST)

Before doing any work on this project (code, refactor, feature, fix), read
`goals.md` at the repo root in full. It tracks what has been completed, current
in-progress state, known tech debt, and open next steps. After any meaningful
change, update its "Joriy holat" (current state) and "Keyingi qadamlar" (next
steps) sections rather than leaving it stale.
