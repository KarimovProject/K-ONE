...
from apps.notifications.forms import EventReminderForm, TelegramChannelSettingsForm
...
class TelegramSettingsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        is_admin = _is_telegram_admin(self.request.user)
        context.update(
            {
...
class TelegramLinkView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        if not cache.add(key, "1", timeout=60):
            messages.warning(request, _("Please wait before creating another link."))
...
class TelegramTestView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        if not cache.add(key, "1", timeout=60):
            messages.warning(request, _("Please wait before sending another test."))
            return redirect("notifications:telegram-settings")
        ...
        try:
            ...
            messages.error(request, _("Test message could not be sent."))
        else:
...
class TelegramChannelSettingsView(LoginRequiredMixin, FormView):
    template_name = "notifications/telegram_channel_settings.html"
    ...
    def get_object(self) -> TelegramChannelSettings:
        return TelegramChannelSettings.load()
    ...
    def form_valid(self, form):
        ...
        settings_obj.save()
        log_audit_event(
            "telegram.channel_settings_updated",
            actor=self.request.user,
            target=settings_obj,
        )
        messages.success(self.request, _("Telegram channel settings updated."))
        return redirect("notifications:telegram-channel-settings")
...
class EventReminderSettingsView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        form.save()
        messages.success(self.request, _("Telegram reminder policy updated."))
