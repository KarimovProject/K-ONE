...
<section class="auth-brand-panel" aria-hidden="true">
  <div class="auth-brand-copy">
    <div class="logo-container">
      <img src="{% static 'img/k-one/k-one-3d-transparent.png' %}" alt="K-ONE Logo" class="brand-logo-large">
    </div>
...
.logo-container-mobile {
  ...
  background-image: radial-gradient(circle at 50% 45%, #0A2F64 0%, #051429 80%), url('../img/k-one/k-one-login-network.svg');
...
.auth-brand-panel::before {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(circle at 30% 70%, rgba(22, 188, 235, 0.12) 0%, rgba(20, 92, 255, 0.06) 50%, transparent 75%);
...
.brand-logo-large {
  ...
  object-fit: contain;
  display: block;
  filter: drop-shadow(0 12px 36px rgba(0, 174, 239, 0.22));
  animation: fadeInScale 0.4s var(--ease-out);
}
...
print("Applied single text and harmonious full-bleed background styling.")
