...
OUT_DIR = r"C:\IEMS\tests\visual_baseline\phase27_2_owner_review"
...
async def assert_authenticated(page, url, expected_marker_selector=None):
    ...
    # Phase 27.2: Assert main content is present and takes up space
    main_content = await page.query_selector(".shell-main, .page-content, main")
    if main_content:
        box = await main_content.bounding_box()
        if not box or box["width"] < 200 or box["height"] < 200:
            print(f"[FAIL] {url} - Workspace main content is missing or too small (collapsed).")
            return False
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("PHASE 27.2 CAPTURE COMPLETE")
