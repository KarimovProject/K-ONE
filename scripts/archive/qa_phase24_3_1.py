...
OUT = Path("tests/visual_baseline/phase24_3_1")
...
def main():
    ...
    try:
        wait_server()
        print("Server running on port 8599, starting Playwright QA for Phase 24.3.1...")
        ...
        with sync_playwright() as p:
            ...
            for item in items:
                ...
                # Parse displayed percentage (e.g., "0,0%" or "1.1%")
                ...
                # Strict tolerance check: 0.1% or subpixel tolerance (0.5px)
                # 0% must explicitly have fill_width == 0
                ...
                print(row_str)
                table_rows.append((code, val_text, f"{track_width:.2f}px", f"{fill_width:.2f}px", f"{visual_pct:.2f}%", status))

            # 2. Capture Screenshots for Phase 24.3.1
