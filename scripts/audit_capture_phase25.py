"""Audit capture script to start server, seed QA data, and take screenshots across viewports for Phase 25."""
...
OUT = Path("docs/audit_screenshots_phase25_1")
...
def prepare():
    ...
    return admin, owner, events, types, venues, org
...
def main():
    ...
    admin, owner, events, types, venues, org = prepare()
        ...
        with sync_playwright() as p:
            ...
            for key, path in routes_anon.items():
                ...
                for vp_name, w, h in VIEWPORTS:
                    ...
                    page.screenshot(path=str(OUT / f"{key}_{vp_name}.png"), full_page=True)
            ...
            routes_auth = {
                ...
                "F_events_list": "/events/",
                "G_event_create_wizard": "/events/wizard/",
                "H_event_detail": f"/events/{events[0].id}/",
                "I_event_edit": f"/events/{events[0].id}/edit/",
                "J_approvals": "/events/approvals/",
                "K_displaced": "/events/displaced/",
                "L_calendar": "/calendar/",
                "M_master_venues": "/master-data/venues/",
                "N_master_venues_new": "/master-data/venues/new/",
                "O_master_event_types": "/master-data/event-types/",
                "P_master_organizations": "/master-data/organizations/",
                "Q_master_sponsors": "/master-data/sponsors/",
                "R_events_speakers": "/events/speakers/",
                "S_reports": "/reports/",
                "T_leadership_dashboard": "/leadership/",
                "U_profile": "/profile/",
                "V_telegram_settings": "/notifications/telegram/",
                "W_publications": "/publications/",
            }
            ...
            for key, path in routes_auth.items():
                ...
                for vp_name, w, h in VIEWPORTS:
                    ...
                    scroll_width = auth_page.evaluate("document.documentElement.scrollWidth")
                    client_width = auth_page.evaluate("document.documentElement.clientWidth")
                    if scroll_width > client_width:
                        print(f"  [OVERFLOW] {key} at {vp_name}")
                        
                    auth_page.screenshot(path=str(OUT / f"{key}_{vp_name}.png"), full_page=True)
